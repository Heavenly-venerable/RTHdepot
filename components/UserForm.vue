...
const userSchema = z.object({
  ...
  password: z.string().min(8, "Password minimal 8 karakter").regex(/[A-Z]/, "Harus ada huruf besar (A-Z)").regex(/[a-z]/, "Harus ada huruf besar (a-z)").regex(/[0-9]/, "Harus ada angka (0-9)").regex(/[^A-Za-z0-9]/, "Harus ada simbol (misalnya !@#$%)"),
})
...
async function onFormSubmit() {
  if (validateForm()) {
    createUser(form)
    toast.add({ severity: 'success', summary: 'Ditambah', detail: 'User berhasil ditambahkan', life: 3000 });
